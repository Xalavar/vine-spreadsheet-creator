...
exports.cleanUpObjectData = cleanUpObjectData;
exports.ejsSheet = ejsSheet;
exports.exportSettingsToJson = void 0;
exports.extractDataFromFile = extractDataFromFile;
var pdfjsLib = _interopRequireWildcard(require("pdfjs-dist"));
var _filterTypes = require("react-table/src/filterTypes");
var _exceljs = _interopRequireDefault(require("exceljs"));
var _UniversalItems = require("./UniversalItems");
var _jszip = _interopRequireDefault(require("jszip"));
var _papaparse = _interopRequireDefault(require("papaparse"));
var _IndexedDB_Functions = require("./IndexedDB_Functions");
var _SettingsTreeFunctions = require("./SettingsTreeFunctions");
...
const NewItemPositions = {
  ...
  '515.4': 'orderDate',
  '592.4': 'dateShipped',
  '669.4': 'orderStatusDate',
  // "Cancelled Date"
  '746.4': 'etv'
};
const errorMessages = {
  'NoDataFound': {
    'shortMsg': 'No Vine data found.',
    'longMsg': 'You likely imported a file with no discernible Vine data. Remember only to import Itemized Reports that you downloaded from your Vine Account tab.'
  },
  'InvalidPDFException': {
    'shortMsg': 'Corrupted or invalid PDF.',
    'longMsg': 'This PDF is either corrupted or contains broken data that cannot be parsed. The only supported PDFs are Itemized Reports.'
  },
  'InvalidFileType': {
    'shortMsg': 'Invalid or unsupported file type.',
    'longMsg': 'Email and spreadsheets are not supported yet. The only currently supported filetypes are: PDFs.'
  }
};
...
function convertDateString(dateString) {
  ...
  if (match1) {
    ...
    const month = new Date(Date.parse("".concat(monthStr, " 1, 2000"))).getMonth() + 1; // Convert month abbreviation to number
...
function csvHandler(buffer) {
  return new Promise((resolve, reject) => {
    const test = _papaparse.default.parse(buffer, {
      header: true,
    ...
    workbook.xlsx.load(buffer).then(function (file) {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        row.eachCell((cell, colNumber) => {
          ...
          rowData["Column".concat(colNumber)] = cell.value;
        });
...
function addStatSheet(wb, years, dataTypeIndices) {
  ...
  */
  ...
  let c_Arr;
  let calcRowLength = 2;
  const odc = (0, _UniversalItems.columnIndexToLetter)(dataTypeIndices.indexOf('orderDate') + 1); // "Order Date Column"
  const ec = (0, _UniversalItems.columnIndexToLetter)(dataTypeIndices.indexOf('etv') + 1); // "ETV Column"
  const osc = (0, _UniversalItems.columnIndexToLetter)(dataTypeIndices.indexOf('orderStatus') + 1); // "Order Status Column"
  ...
  for (let y = currYear; y > 2006; y--) {
    ...
    if (years.includes(y)) {
      ...
      for (let i = 1; i < 13; i++) {
        cellFormula = i === 12 ? "=COUNTIFS('".concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=\"&DATE(").concat(y, ",").concat(i, ",1),'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<\"&DATE(").concat(y + 1, ",1,1))") : "=COUNTIFS('".concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=\"&DATE(").concat(y, ",").concat(i, ",1),'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<\"&DATE(").concat(y, ",").concat(i + 1, ",1))");
      ...
      row.push(y);
      for (let i = 1; i < 8; i++) {
        cellFormula = "=SUMPRODUCT(('".concat(mainSheet, "'!").concat(odc, "2:INDEX('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",COUNTA('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, "))>=DATE(").concat(y, ",1,1))*('").concat(mainSheet, "'!").concat(odc, "2:INDEX('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",COUNTA('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, "))<DATE(").concat(y + 1, ",1,1))*(WEEKDAY('").concat(mainSheet, "'!").concat(odc, "2:INDEX('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",COUNTA('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ")),1)=").concat(i, "))");
      ...
      row.push(y);
      for (let i = 0; i < 24; i++) {
        cellFormula = "=SUMPRODUCT(('".concat(mainSheet, "'!").concat(odc, "2:INDEX('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",COUNTA('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, "))>=DATE(").concat(y, ",1,1))*('").concat(mainSheet, "'!").concat(odc, "2:INDEX('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",COUNTA('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, "))<DATE(").concat(y + 1, ",1,1))*(HOUR('").concat(mainSheet, "'!").concat(odc, "2:INDEX('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",COUNTA('").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ")))=").concat(i, "))");
        //cellFormula = `=SUMPRODUCT(('${mainSheet}'!${odc}2:INDEX('${mainSheet}'!${odc}:${odc},COUNTA('${mainSheet}'!${odc}:${odc}))>=DATE(${y},1,1))*('${mainSheet}'!${odc}2:INDEX('${mainSheet}'!${odc}:${odc},COUNTA('${mainSheet}'!${odc}:${odc}))<=DATE(${y},12,31))*(WEEKDAY('${mainSheet}'!${odc}2:INDEX('${mainSheet}'!${odc}:${odc},COUNTA('${mainSheet}'!${odc}:${odc})),1)=${d}))`;
      ...
      // by etv range
      row.push(y);
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ",0,'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ",\">=0.01\",'").concat(mainSheet, "'!").concat(ec, ":").concat(ec, ",\"<=100\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ",\">100\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      etvRows.push(row);
      ...
      cellFormula = "=SUMIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \">=1/1/").concat(y, "\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=SUMIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ", '").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ", \"Cancelled\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \">=1/1/").concat(y, "\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \"<=12/31/").concat(y, "\") * -1");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=SUMIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ", '").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ", \"Defective\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \">=1/1/").concat(y, "\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \"<=12/31/").concat(y, "\") * -1");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=SUMIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ", '").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ", \"<>*None\", '").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ", \"<>*Cancelled\", '").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ", \"<>*Defective\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \">=1/1/").concat(y, "\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \"<=12/31/").concat(y, "\") * -1");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=SUMIFS('".concat(mainSheet, "'!").concat(ec, ":").concat(ec, ", '").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ", \"None\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \">=1/1/").concat(y, "\", '").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ", \"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      ...
      row.push(y);
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(osc, ":").concat(osc, ",\"None\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(osc, ":").concat(osc, ",\"Cancelled\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(osc, ":").concat(osc, ",\"Defective\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(osc, ":").concat(osc, ",\"<>*None\",'").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ",\"<>*Cancelled\",'").concat(mainSheet, "'!").concat(osc, ":").concat(osc, ",\"<>*Defective\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      // accounts for anything else, like "Other" or a custom input
      row.push({
        formula: cellFormula
      });
      cellFormula = "=COUNTIFS('".concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\">=1/1/").concat(y, "\",'").concat(mainSheet, "'!").concat(odc, ":").concat(odc, ",\"<=12/31/").concat(y, "\")");
      row.push({
        formula: cellFormula
      });
  ...
  function t_title(sheet, c, r, title) {
    //const
    sheet.mergeCells("B ".concat(r, ":").concat(c).concat(r));
    sheet.getCell("".concat(c).concat(r)).value = title;
    sheet.getCell("".concat(c).concat(r)).font = {
      bold: true
    };
    sheet.getCell("".concat(c).concat(r)).alignment = {
      horizontal: 'center'
    };
  }
...
async function ejsSheet() {
  ...
  const database = await (0, _IndexedDB_Functions.PerformDbOperation)('getAll');
  ...
  function createExcelSheet(dataAsArray) {
    ...
    const tableColumns = dataColumnConfigs.map((item, index) => {
      ...
      const test = _UniversalItems.DataTypes.find(subitem => subitem.value === item.dataType);
      const headerName = item !== null && item !== void 0 && item['displayName'] ? item['displayName'] : _UniversalItems.DataTypes.find(subitem => subitem.value === item.dataType)['name']; //DataTypes.find(subitem => subitem.dataType === item.dataType);
      const fbSetting = !!item.dataType.match(/(product name|date|etv)/i);
      return {
        name: headerName,
        filterButton: fbSetting
      };
    });
    ...
    const table = worksheet.addTable({
      ...
      headerRow: true,
      style: {
        theme: userSettings !== null && userSettings !== void 0 && userSettings['tableTheme'] ? userSettings['tableTheme'] : null,
        showRowStripes: true
      },
    ...
    dataColumnConfigs.forEach((columnSettings, index) => {
      ...
      worksheet.getColumn((0, _UniversalItems.columnIndexToLetter)(i)).width = columnSettings !== null && columnSettings !== void 0 && columnSettings['colWidth'] ? columnSettings['colWidth'] : 12;
      worksheet.getColumn((0, _UniversalItems.columnIndexToLetter)(i)).eachCell({
        includeEmpty: true
      }, (cell, rowNumber) => {
        if (cell.value) {
          let url, tooltip;
            ...
            if (columnSettings.dataType.match(/(date)/i)) {
              ...
              if (!cell.value.includes('amazon')) {
                ...
                worksheet.addImage(imageId, {
                  ...
                  // need to figure out how to discern column later
                  ext: {
                    width: 79,
                    height: 79
                  }
              ...
              if (cell.value.includes("-")) {
                // default order page
                url = "https://www.amazon.com/gp/your-account/order-details?orderID=".concat(cell.value);
              } else if (cell.value.length > 0) {
                // the unique Vine order page
                //let asin = worksheet.getCell(`B ${rowNumber}`);
                const c = (0, _UniversalItems.columnIndexToLetter)(dataColumnConfigs.findIndex(item => item.asin === "asin"));
                let asin = worksheet.getCell("".concat(c, " ").concat(rowNumber));
                url = "https://www.amazon.com/vine/orders/".concat(cell.value, "?enrollment-asin=").concat(asin);
              }
              /*
              if (url) {
  ...
  try {
    ...
    switch (type) {
      case 'all':
        ...
        break;
      case 'style':
        // export only spreadsheet styling-related settings
        const {
          tableTheme,
          columnManagement,
          columnDefaults
...
exports.exportSettingsToJson = exportSettingsToJson;
  ...
  zip.loadAsync(file).then(function (contents) {
    ...
    contents.forEach(function (relativePath, file) {
      ...
      file.async('string').then(function (content) {
        ...
        document.getElementById('output').innerHTML += "<p>".concat(relativePath, ": ").concat(content, "</p>");
      });
    });
  }).catch(function (error) {
...
//# sourceMappingURL=FileParsingMethods.js.map